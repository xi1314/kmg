...
var Psk: [UInt8] = []
func sendRequest(apiName:String,params:Dictionary<String, AnyObject>,callback:(JSON)->Void){
if params.count != 0{//TODO:将有参数和无参数重构为一种
HttpSwift.rpcPost(self.RemoteUrl,apiName:apiName,psk:self.Psk, params:params,callback: {(data,response,error) in
if error != nil{
print(error)
return
}
var inData = [] as [UInt8]
if self.Psk.count != 0{
inData = DecryptBtyesDecode(self.Psk, data: data!).arrayOfBytes()
}else{
inData = data!.arrayOfBytes()
}
let filterByte = inData.filter{value in
if value == 2{
return false
}
return true
}
let filterData = NSData(bytes: filterByte as [UInt8], length: filterByte.count)
let outData = JSON(data:filterData)
callback(outData)
})
}else{
HttpSwift.rpcPost(self.RemoteUrl,apiName:apiName,psk: self.Psk, callback: {(data,response,error) in
if error != nil{
print(error)
return
}
...
static func ConfigDefaultClient(RemoteUrl: String,pskStr: String?)->Client{
let newclient = Client()
newclient.RemoteUrl = RemoteUrl
if pskStr != nil{
newclient.Psk = Array((pskStr?.nsdata.sha512()?.arrayOfBytes())![0...31])
}
return newclient
}
}
let magicCode4 = [167, 151, 109, 21] as [UInt8]
extension String {
var nsdata: NSData {
...
static func rpcPost(url: String, apiName: String,psk: [UInt8], callback: (data: NSData!, response: NSURLResponse!, error: NSError!) -> Void) {
let manager = HttpSwiftManager(url: url, method: "POST", callback: callback)
...
/*POST请求，带参数*/
static func rpcPost(url: String, apiName: String, psk: [UInt8], params: Dictionary<String, AnyObject>, callback: (data: NSData?, response: NSURLResponse?, error: NSError?) -> Void) {
let manager = HttpSwiftManager(url: url, method: "POST", params: params, callback: callback)
manager.apiName = apiName
manager.psk = psk
manager.fire()
...
/*PUT请求，不带参数*/
static func put(url: String,  callback: (data: NSData!, response: NSURLResponse!, error: NSError!) -> Void) {
let manager = HttpSwiftManager(url: url, method: "PUT", callback: callback)
manager.fire()
}
/*PUT请求，带参数*/
static func put(url: String, params: Dictionary<String, AnyObject>, callback: (data: NSData!, response: NSURLResponse!, error: NSError!) -> Void) {
let manager = HttpSwiftManager(url: url, method: "PUT", params: params, callback: callback)
manager.fire()
}
/*DELETE请求，带参数*/
static func delete(url: String, params: Dictionary<String, AnyObject>, callback: (data: NSData!, response: NSURLResponse!, error: NSError!) -> Void) {
...
func buildBody() {
let apiName =  self.apiName.dataUsingEncoding(NSUTF8StringEncoding)?.arrayOfBytes()
let apiNameLength = [UInt8.init((apiName?.count)!)]
var inData: [UInt8]
if self.params.count > 0 && self.method != "GET" {
inData = buildParams(self.params).nsdata.arrayOfBytes()
}else{
inData = ("{}".dataUsingEncoding(NSUTF8StringEncoding)?.arrayOfBytes())!
}
let originData = NSData.withBytes( apiNameLength + apiName! + inData)
if self.psk.count != 0{
request.HTTPBody = EncryptBtyesDecode(self.psk, data: originData)
}else{
...
return JSONStringify(parameters, prettyPrinted: true)
}
...
func join(joinCode:String,var StringArray:[String])->String{
var out = ""
while StringArray.last != nil{
let str = StringArray.popLast()
out += str! + "&"
}
return out
}
...
func DecryptBtyesDecode(key:[UInt8],data:NSData)->NSData{
let inData = data.arrayOfBytes()
if inData.count<16+4+1{
print("input data too small")
return data
}
var resultOut = NSData()
do {
var output = try AES(key: key, iv: Array(inData[0...15]), blockMode: .CTR)!.decrypt(Array(inData[16...(inData.count-1)]))
output = Array(output[0...(inData.count-1-16)])
if magicCode4 != Array(output[(output.count-4)...(output.count-1)]){
print("magicCode not match")
return resultOut
}
output = Array(output[0...(output.count-5)])
let mark = output[0]
if mark == 0 {
if output[1] == 2{
resultOut = NSData.withBytes(Array(output[1...(output.count-1)]))
}
}else{
let outData = NSData.withBytes(Array(output[1...(output.count-1)]))
resultOut = outData.zlibInflate()
}
}catch{
...
func EncryptBtyesDecode(key:[UInt8],data:NSData)->NSData{
var inData = data.zlibDeflate().arrayOfBytes()
if inData.count>data.arrayOfBytes().count{
inData = [0] + data.arrayOfBytes()
}else{
inData = [1] + inData
}
let Iv = Cipher.randomIV(16)
var outData = NSData()
do {
let ss = Iv+inData+magicCode4
let inDataV2 = Array(ss[16...(ss.count-1)])
var output = try Cipher.AES(key: key, iv: Iv, blockMode: .CTR).encrypt(inDataV2)
output = Iv+output
outData = NSData.withBytes(output)
}catch{
